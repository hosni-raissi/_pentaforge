...
\usepackage{graphicx}
\usepackage{float}
\usepackage{cite} % sorts and compresses numeric citations (e.g., [5,7] not [7,5])
...
% Better justification with fewer ugly hyphen splits
\usepackage{microtype}
% Avoid breaking words with hyphens (move whole word to next line instead)
\hyphenpenalty=10000
...
\setotherlanguage{french}  % <--- ADD THIS FOR YOUR RÉSUMÉ
...
\begin{titlepage}
    ...
    {\Large \textbf{``AI-Powered Automated Penetration Testing''}\par}
    ...
    {\normalsize \textit{Professional Supervisor:} Mr. Walid Bensaid\par}
    \vspace{0.1cm}
...
\centering
Professional Supervisor, \textbf{Mr.\ Walid Bensaid}
...
To my family, the unwavering pillar of my life: this work is dedicated to you.
...
To my two brothers and my sister, companions at all times who shared the laughter,
the challenges, and the aspirations: this work is dedicated to you.
Your camaraderie and support enriched every moment of this adventure.

\vspace{1cm}

To my close friends who have always been there: your friendship, encouragement,
and advice played a crucial role in my journey. Your presence and feedback often
guided my choices when it mattered most.
...
to supervise this work on behalf of ENICarthage. His advice and recommendations
...
Traditional penetration testing engagements remain resource-intensive and difficult to scale. In practice, workflows are still fragmented across specialized tools, with findings and context transferred manually between phases~\cite{autosecagent2026,emazzantiTools}. Session knowledge is often lost across interruptions, which forces repeated reconstruction of the attack chain~\cite{pentestgpt2023}. Retesting after remediation is also frequently incomplete, allowing implementation errors and bypass variants to slip through validation~\cite{hedgehogRetesting,redleggRetesting,halockVerification}. These limitations are not the result of poor practice; they are structural constraints of a fundamentally manual process~\cite{cycognitoPentest}.
...
\begin{itemize}
    ...
    \item \textbf{Chapter 1 --- ``General Framework of the Project'':}
    ...
    \item \textbf{Chapter 3 --- ``Specification and Architecture'':}
...
\begin{figure}[H]
    \centering
    % Image source: https://share.google/cRQadkwXqYaxIiegY
    \includegraphics[width=0.3\textwidth]{talan1.png}
    \caption{Talan Logo \cite{talanLogo}}
...
\begin{itemize}
    ...
    \item \textbf{Cybersecurity:} Consulting in cybersecurity, compliance, and
...
The platform coordinates a set of specialized AI agents (Planner, Executor,
...
\begin{figure}[H]
    \centering
    % Image source: https://share.google/5HtFyjvGZg3le6mXj
    \includegraphics[width=0.9\textwidth]{blog-scrum-process-opt.jpg}
    \caption{Scrum Development Process \cite{pmpartnersScrumProcess}}
    \label{fig:scrum}
\end{figure}
...
\end{itemize}
    ...
    \begin{itemize}
        \item Develop the Executor agent responsible for dispatching and running
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{sprint_timeline.png}
    \caption{PentaForge sprint timeline across the 16-week internship}
...
The next chapter examines the state of the art by exploring the role of artificial
...
\setcounter{section}{0}
% Reset float counters because chapter headings are manual (no \chapter command)
\setcounter{table}{0}
\setcounter{figure}{0}
...
exploit, and proposing a remediation fix entirely  within an automated pipeline. This
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        node distance=8mm,
        phase/.style={draw, rounded corners, align=center, font=\small, inner xsep=8pt, inner ysep=6pt},
        arr/.style={-Latex, thick}
    ]
        \node[phase] (recon) {Reconnaissance};
        \node[phase, below=of recon] (scan) {Scanning \\ \& Enumeration};
        \node[phase, below=of scan] (exploit) {Exploitation};
        \node[phase, below=of exploit] (post) {Post-Exploitation};
        \node[phase, below=of post] (report) {Reporting};
    ...
    \end{tikzpicture}
    \caption{Traditional penetration testing lifecycle phases (PTES-inspired)}
    \label{fig:pentest_lifecycle}
\end{figure}
...
that is, coherent attack chains that simulate the behavior of a real attacker. This
...
This section reviews five representative AI-assisted penetration testing solutions.
Each tool is assessed against a shared set of evaluation criteria to make the comparison
consistent and to surface differences in scope, autonomy, evidence handling, and integration maturity.

\subsection{PentestGPT}

PentestGPT~\cite{pentestgpt2023} is an open-source solution that integrates an LLM
into a pentester's workflow through a command-line interface. Its architecture relies on
a three-module reasoning loop: a Reasoning Module for planning, a Generation Module for
command and payload creation, and a Parsing Module that interprets tool outputs and
feeds them back into the reasoning layer. Because it functions primarily as an interactive co-pilot, the human operator remains responsible for manually executing the suggested commands and pasting the results back into the prompt, which limits its true operational autonomy.
...
adding AI-assisted features to its web application penetration testing workflow. The
Explore Issue feature autonomously investigates scanner findings by attempting
exploitation steps and uncovering additional attack vectors. In addition, an
AI-assisted false positive reduction mechanism filters broken access control false
positives before reporting, reducing manual verification effort. Unlike standalone autonomous agents, Burp AI is deeply embedded within a traditional proxy-based workflow, serving as an advanced accelerator for human-led testing rather than an independent orchestrator.
...
KaliGPT~\cite{kaligpt} integrates an LLM into Kali Linux to support the execution and
interpretation of penetration testing commands through a structured workflow. Its
\texttt{run} pattern routes each tool execution through AI analysis without requiring
manual copy-paste. While this streamlines the immediate execution-to-analysis loop, the tool acts primarily as an intelligent command-line wrapper rather than an autonomous planner capable of multi-step strategic reasoning.
...
positioned for enterprise use. It provides end-to-end automation across multiple
attack surfaces, impact-based prioritization, and executive-level reporting. It operates purely as a black-box testing engine, leveraging vast proprietary datasets to autonomously discover and chain vulnerabilities across complex enterprise networks.
...
Keygraph for web applications and APIs. It combines static source code analysis with
live exploitation: it analyzes the application's source to identify attack vectors,
then dispatches dedicated exploit agents (via Playwright browser automation) to
confirm each hypothesis with a working proof of concept (PoC). Shannon enforces a
strict ``PoC or it didn't happen'' policy: no vulnerability is reported without a
verified exploit, and the project reports a high exploit verification success rate on
...
\textbf{Limitations:} exclusively white-box, as it requires access to source code,
...
In the following, we build a comparative matrix that maps each solution to a common
set of evaluation criteria. This allows us to identify which capabilities are already
well addressed in the market and which gaps remain. These gaps directly motivate the
functional priorities and architectural choices retained for PentaForge.
...
an open-source, on-premise, multi-agent platform that simultaneously combines all the following properties: autonomous task orchestration, persistent engagement
...
of Burp Suite AI. Furthermore, the platform is deployable entirely on-premise with no sensitive data leaving
...
To objectively determine the most suitable reasoning backbone for PentaForge, a comparative benchmark was conducted across three distinct categories of Large Language Models. \textbf{Gemini 2.5 Flash} was selected to represent high-throughput, massive-context API solutions; \textbf{Mistral Large 3} was evaluated as a high-performing open-weight alternative; and \textbf{Qwen 2.5 7B} was chosen to test the viability of a fully local, privacy-first model capable of running on consumer-grade hardware. The evaluation, synthesized in Table \ref{tab:llm_bench}, aligns directly with the architectural constraints identified previously: it measures each model's capacity to handle heavy token budgets (TPM), retain deep engagement memory, perform structured security reasoning, and meet the strict economic and confidentiality requirements of penetration testing.
\begin{table}[H]
    \centering
    ...
    \begin{tabular}{|p{0.24\textwidth}|
        ...
        15 RPM (Free tier)~\cite{geminiRateLimits} &
        $\sim$60 RPM (Free mode) &
        Hardware-bound \\
        \hline
        Tokens per minute (TPM) &
        1{,}000{,}000 TPM~\cite{geminiRateLimits} &
        $\sim$500{,}000 TPM &
        Hardware-bound \\
        \hline
        Requests per day (RPD) &
        1{,}500 RPD (Free tier)~\cite{geminiRateLimits} &
        $\sim$1B tokens/month &
        Unlimited \\
        \hline
...
peak token requirement of approximately 33{,}000 to 132{,}000 TPM, which is well within
...
proprietary, meaning target data transits Google's infrastructure, and 
the cost of \$2.50 per million output tokens accumulates across sustained
engagement cycles. For cost-sensitive or privacy-critical deployments, the local fallback
...
This section examines the four foundational technologies that collectively
enable the design of PentaForge. For each technology, we compare the
available approaches, justify the design choice retained, and identify the
specific limitations that motivated it. This comparative grounding
distinguishes the choices made in PentaForge from simpler alternatives and
clarifies why each technology is necessary rather than merely convenient.

\subsection{Large Language Models and Reasoning Strategies}

\subsubsection{Approaches Compared}

Three families of AI-driven reasoning are applicable to automated penetration
testing: rule-based expert systems, classical machine learning classifiers,
and large language model (LLM) reasoning.
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{0.22\textwidth}|p{0.36\textwidth}|p{0.30\textwidth}|}
        \hline
        \textbf{Approach} & \textbf{Capability} & \textbf{Limiting Factor} \\
        \hline
        Rule-based expert systems & Deterministic, auditable, fast on known
        patterns & Cannot generalize to novel targets or
        unknown vulnerability classes \\
        \hline
        Classical ML classifiers & Strong on structured tabular data and
        known malware families & Require labeled training data; brittle on
        out-of-distribution inputs \\
        \hline
        LLMs with Chain-of-Thought & Generalize across targets, generate
        structured attack plans, interpret raw tool output, produce
        remediation language & Higher inference cost; require careful
        prompt design to avoid hallucination \\
        \hline
    \end{tabular}
    \caption{Comparison of AI reasoning approaches for penetration testing}
    \label{tab:llm_approaches}
\end{table}

\subsubsection{Choice and Rationale}

PentaForge adopts LLMs with Chain-of-Thought (CoT) prompting as its
reasoning backbone. Rule-based systems are unsuitable because each
engagement surface is different: a web application target, a cloud
environment, and an IoT device each require distinct attack reasoning that
no static ruleset can cover exhaustively. Classical classifiers are likewise
insufficient because they depend on labeled historical data and cannot
produce structured, multi-step attack plans from a natural language target
description.

LLMs with CoT prompting address both limitations. By reasoning
step-by-step before producing output, they generate coherent attack plans
for novel or complex targets where no prior labeled example exists.
PentaForge's Planner agent uses this capability explicitly: for engagements
that fall outside standard PTES templates, the Slow Path activates CoT
reasoning to derive a target-specific scenario sequence rather than falling
back to a generic checklist.

The key tradeoff accepted here is inference cost and hallucination risk.
PentaForge mitigates the latter through its Verification tier in the
Analyzer, which requires structured evidence before any finding is promoted
to verified status, preventing CoT-generated hypotheses from reaching the
report unchecked.

\subsection{Knowledge Currency: RAG versus Fine-Tuning versus Static
Prompting}

\subsubsection{Approaches Compared}

Three strategies exist for keeping a language model current with evolving
security knowledge: periodic fine-tuning on new vulnerability data,
embedding domain knowledge in static system prompts, and
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{0.22\textwidth}|p{0.36\textwidth}|p{0.30\textwidth}|}
        ...
        \textbf{Approach} & \textbf{Capability} & \textbf{Limiting Factor} \\
        \hline
        Periodic fine-tuning & Deeply integrates new knowledge into model
        weights & Expensive, slow, and requires curated labeled datasets;
        knowledge becomes stale between runs \\
        \hline
        Static system prompts & Zero infrastructure overhead; immediate to
        update & Context window fills quickly; cannot scale to the breadth
        of CVE and TTP knowledge required \\
        \hline
        RAG with vector retrieval & Decouples knowledge from model weights;
        retrieves only what is relevant per query; continuously updatable
        without retraining & Retrieval quality depends on embedding model
        and chunking strategy \\
        \hline
    \end{tabular}
    \caption{Comparison of knowledge-currency strategies for LLM-based systems}
    \label{tab:rag_approaches}
\end{table}

\subsubsection{Choice and Rationale}

PentaForge adopts RAG with a Qdrant vector database as its knowledge
currency mechanism. Fine-tuning is ruled out because new CVEs are published
daily and retraining a model on that cadence is operationally and
financially unsustainable. Static system prompts are insufficient because
the breadth of CVE entries, WAF bypass patterns, MITRE ATT\&CK TTPs, and
target-type-specific guidance far exceeds what can be held in a context
window without crowding out engagement-specific reasoning.

RAG decouples knowledge from model weights, injecting only the retrieved
context that is semantically relevant to the current query. This means the
platform's security intelligence remains current through continuous index
updates without touching the underlying model. The tradeoff accepted is
that retrieval quality depends on the embedding model and chunking strategy.
PentaForge addresses this by maintaining per-domain vector indexes, specifically
shared knowledge, target-type resources, project findings, and report
context, so that retrieval remains focused and relevance scores remain
meaningful.

\subsection{Agent Coordination: Single Agent versus Multi-Agent Architectures}

\subsubsection{Approaches Compared}

Two dominant architectures exist for LLM-driven task automation: a single
monolithic agent that handles all phases of a task, and a multi-agent
system (MAS) where specialized agents collaborate through a shared state.
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{0.22\textwidth}|p{0.36\textwidth}|p{0.30\textwidth}|}
        ...
        \textbf{Approach} & \textbf{Capability} & \textbf{Limiting Factor} \\
        \hline
        Single monolithic agent & Simpler to implement and debug; lower
        coordination overhead & Context window saturates quickly on long
        engagements; a single failure halts the entire pipeline; no
        specialization \\
        \hline
        Multi-agent system & Specialized agents per phase; parallel execution
        on independent targets; fault isolation; shared state enables
        cross-agent memory & Higher design complexity; requires explicit
        orchestration and shared-state discipline \\
        \hline
    \end{tabular}
    \caption{Comparison of single-agent versus multi-agent coordination strategies}
    \label{tab:mas_approaches}
\end{table}

\subsubsection{Choice and Rationale}

PentaForge adopts a multi-agent architecture. The penetration testing
lifecycle is not a single uniform task: reconnaissance, exploitation,
verification, and reporting each require different tool sets, reasoning
strategies, and output formats. A single monolithic agent would need to
hold the full context of all phases simultaneously, saturating its context
window during deep engagements and making failure recovery structurally
difficult.

The MAS approach allows each agent to operate within a focused context
window, receive only the memory projections relevant to its phase, and
fail independently without blocking the rest of the pipeline. PentestGPT,
the closest open-source alternative, uses a three-module reasoning loop
within a single agent and explicitly suffers from context saturation on
long sessions~\cite{pentestgpt2023}. NodeZero demonstrates that a MAS
approach can deliver end-to-end automation at commercial scale, confirming
that the architecture is viable for the domain.

The tradeoff accepted is coordination complexity. PentaForge addresses
this through its centralized Scan Orchestrator, which resolves
inter-agent dependencies, sequences phase transitions, and persists shared
state so that each agent always reads from a consistent engagement
snapshot.

\subsection{Vulnerability Prioritization: CVSS versus SSVC}

\subsubsection{Approaches Compared}

Two dominant frameworks govern vulnerability prioritization in security
assessments: the Common Vulnerability Scoring System (CVSS) and
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{0.22\textwidth}|p{0.36\textwidth}|p{0.30\textwidth}|}
        \hline
        \textbf{Framework} & \textbf{Capability} & \textbf{Limiting Factor} \\
        \hline
        CVSS & Universally recognized; provides a standardized numeric
        severity score; widely supported by scanners and reporting tools &
        Context-free: assigns the same score regardless of the deployment
        environment, exploitability status, or mission criticality of the
        affected asset \\
        \hline
        SSVC & Incorporates exploitation state, technical impact,
        automatable exploitation likelihood, and mission prevalence;
        produces an actionable decision (ACT, ATTEND, TRACK) rather than
        a score & Less widely known; requires enrichment data from NVD,
        CISA KEV, and EPSS \\
        \hline
    \end{tabular}
    \caption{Comparison of CVSS and SSVC vulnerability prioritization frameworks}
    \label{tab:ssvc_approaches}
\end{table}

\subsubsection{Choice and Rationale}

PentaForge retains CVSS scores for compatibility with industry reporting
standards, but adopts SSVC as its primary prioritization mechanism in the
Analyzer. The limitation of CVSS is well-documented in practice: a
CVSS~9.8 vulnerability on an air-gapped test server is operationally less
urgent than a CVSS~6.5 vulnerability on an internet-facing payment API.
However, providing a single number without deployment context can mislead triage
efforts and waste remediation resources.

SSVC, developed by CISA and Carnegie Mellon University, addresses this
directly. By incorporating four contextual dimensions (exploitation
state from NVD and CISA KEV, technical impact from CVSS, automatable
exploitation likelihood from EPSS, and mission prevalence from the asset
inventory), it produces an actionable decision rather than a score.
The output (ACT, ATTEND, or TRACK) tells the security team what to do,
not merely how severe a number is.

The tradeoff accepted is that SSVC requires enrichment data from external
sources. PentaForge addresses this through its Intel Agent, which
continuously refreshes NVD, CISA KEV, and EPSS data into the RAG knowledge
base so that the Analyzer's SSVC decision tree always operates on current
...
The technologies reviewed in the final section, specifically LLMs with Chain-of-Thought
...
product backlog that bridges those requirements into a prioritized delivery plan.
We then present the global architecture in its monolithic, logical, and physical
views, detail the multi-agent execution loop together with the support services
that surround it, and conclude with the technical stack retained for the desktop
client, backend services, storage layer, and sandboxed execution path.
...
workflow rules. \\
\hline
External target system & Respond to reconnaissance, crawling, validation, and safe exploit
...
The following diagram provides a global overview of all interactions between
the platform actors and the system. The four functional groups identified in
the subsequent sections each correspond to a specific zone of this diagram.

\begin{figure}[H]
    \centering
    \makebox[\textwidth][c]{%
        \includegraphics[width=1.2\textwidth]{use_case_diagram.png}%
    }
    \caption{Global use-case diagram of PentaForge}
...
diagram that zooms into the interactions relevant to that functional domain.
% ── 3.1.3.1 ───────────────────────────────────────────────────────────────
...
\begin{itemize}
    ...
    \item The system shall allow the pentester to explicitly define in-scope 
    boundaries and out-of-scope exclusions (e.g., blacklisted IP ranges or sensitive 
    endpoints) to prevent unauthorized automated interactions.
    \item The system shall allow the operator to configure and securely store 
    authentication profiles (e.g., session cookies, API tokens, or login credentials) 
    required for authenticated (grey-box) target assessments.
    ...
    \item The system shall provide a secure deletion mechanism to completely 
    remove all project-related artifacts, vector memory, and execution logs 
    once an engagement is formally closed.
\end{itemize}
\begin{figure}[H]
    \centering
...
\hline
FR12 & Executor & The system shall route tool requests through a controlled
execution layer. \\
\hline
FR13 & Executor & The system shall reject out-of-scope tool requests. \\
\hline
FR14 & Executor & The system shall reuse valid project-scoped tool results
...
The functional, non-functional, and actor-oriented requirements defined in this section collectively specify the contract that PentaForge must fulfill. The following section translates these requirements into a prioritized delivery plan through the product backlog.

% ── Section 2 ─────────────────────────────────────────────────────────────
\section{Product Backlog}

The product backlog translates the functional and non-functional requirements
defined in Section~\ref{sec:requirements} into a set of prioritized,
deliverable user stories. Each story is assigned to a specific sprint,
establishing the concrete execution contract between the requirements
analysis and the architectural design that follows. The backlog is organized
chronologically across the seven sprints of the project; each item carries
a priority level (High, Medium, or Low) reflecting its criticality to the
...
logs are normalized into structured findings. & Executor & High & S3 \\
\hline
...
PentaForge’s conceptual architecture is built upon three foundational pillars:
\begin{itemize}
    \item \textbf{The Human Operator:} Sits at the top of the control hierarchy, defining the project scope and authorizing all target engagements.
    \item \textbf{The AI Orchestration Core:} Acts as the bridge between the operator and the target, mediating all automated tool execution through strict safety guards.
    \item \textbf{The Storage Layer:} Provides the persistent memory necessary to maintain state coherence across system restarts and extended engagements.
\end{itemize}

\begin{figure}[H]
    \centering
    \makebox[\textwidth][c]{%
        \includegraphics[width=1\textwidth]{concept_architecture.png}%
    }
...
The external interfaces are the desktop UI communicating over HTTP and SSE, the sandbox service receiving tool execution requests, and the LLM provider receiving inference calls. All other concerns, including persistence, orchestration, agent reasoning, and retrieval, are internal to the deployment unit.
...
PentaForge eliminates the distributed coordination overhead of inter-service
network calls during the hot path of a scan. When an agent queries project
memory, updates scan state, or triggers a tool execution, these operations
resolve within the same process boundary, reducing latency and simplifying
failure recovery.
...
\mbox{containerized} services on a local host. Each service runs in its own Docker
...
Sequence diagrams capture the temporal dimension of PentaForge's architecture
that static structural views cannot express. While the logical and physical
architectures presented in the previous sections describe \textit{what} exists
and \textit{where} it runs, sequence diagrams answer \textit{how} components
collaborate under real operational conditions: which service initiates a
transaction, what data crosses each boundary, and in what order state
transitions occur. The four diagrams below were selected because they each exercise a distinct and critical path through the platform: scan initiation, failure recovery, report production, and secure client delivery. Collectively, these cover the full engagement lifecycle from the operator’s first action to the client’s final download.

\subsubsection{SD1: Project Scan Launch}

This sequence diagram illustrates the initial phase of an engagement, from
project creation in the desktop UI to the initialization of the scan
orchestrator and the emission of live events.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{sd1_project_scan_launch.png}
    \caption{Sequence diagram for project scan launch}
    \label{fig:sd1_launch}
\end{figure}

\subsubsection{SD2: Resume After Interruption}

This sequence demonstrates the platform's smart resume capability. It details
how the backend recovers the persisted scan state and continues from the exact
checkpoint after a restart or interruption.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{sd2_resume_after_interruption.png}
    \caption{Sequence diagram for resume after interruption}
    \label{fig:sd2_resume}
\end{figure}

\subsubsection{SD3: Report Generation}

This sequence outlines the workflow for transforming analyzed evidence and findings into a professional report. It details the interaction between the Report Generator node, the storage layer, and the UI. To ensure maximum flexibility, the platform automatically generates the report in two distinct formats: a polished HTML version for client delivery and a raw Markdown version for technical review. The operator retains full control over the output, with the ability to manually edit the Markdown content directly within the interface to refine the narrative or adjust recommendations. Furthermore, the report can be dynamically regenerated at any time to seamlessly incorporate newly verified findings without disrupting the ongoing engagement.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{sd3_report_generation.png}
    \caption{Sequence diagram for report generation}
    \label{fig:sd3_report}
\end{figure}

\subsubsection{SD4: Protected Client Download}


The client delivery path is deliberately constrained: the share token encodes
only the report identifier, giving the client access to the scoped portal
with no visibility into scan controls, agent outputs, or project metadata.
The exported deliverable is either a password-protected PDF encrypted at
the document level, or an AES-256 ZIP archive produced via 7-Zip. In both
cases, the password is set by the pentester at export time, never stored by
the platform, and communicated to the client out-of-band.
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{sd4_protected_client_download.png}
    \caption{Sequence diagram for protected client download}
    \label{fig:sd4_download}
\end{figure}
...
scenarios for the executor lane. Its input is heterogeneous by design: it can
...
\textbf{Recon Executor} and \textbf{Exploit Executor} are the two operational agents that constitute the hot execution lane. They qualify as true agents in the formal sense: each one runs an LLM reasoning loop, has access to a scoped tool catalog, and calls tools autonomously via the sandbox client. Both are target-type aware, meaning they receive only the tooling relevant to the current engagement surface and are not permitted to invoke tools outside their assigned catalog. Furthermore, each agent maintains a localized short-term memory that tracks the tools executed in previous cycles along with a concise summary of their findings. This memory mechanism prevents redundant operations by allowing the agent to intelligently skip re-running a tool if the required information has already been acquired.
...
\begin{enumerate}
    ...
    API keys, session tokens, or credentials) using deterministic filters and entropy
    checks.
    ...
    (such as the Executor or Planner).
\end{enumerate}
...
This chapter has translated the theoretical foundations of Chapter 2 into a
concrete specification and architectural design for PentaForge. The requirements
analysis formalized what the platform must do and under what constraints. The
product backlog then operationalized those requirements into a prioritized,
sprint-organized delivery plan, establishing the execution contract before any
design decision was made. The monolithic, logical, and physical architectural views established the structural boundaries from a single deployable unit down to individual containers and their network relationships. The deep
multi-agent design detailed the role and interaction of each specialized
component, from Intel preparation and memory construction to analyzer feedback
and durable recovery paths. Finally, the technical stack justification
demonstrated that every technology choice was made deliberately against evaluated
alternatives.

The following chapter presents the concrete implementation of this architecture: the working platform, its agent orchestration cycle in action, and the results produced on a real test environment.
...
% ── Section 3 ─────────────────────────────────────────────────────────────
\section{Iterative Implementation and Sprint Realizations}

PentaForge was developed iteratively across seven sprints following the Scrum methodology introduced in Chapter 1. Rather than presenting the platform as a single monolithic block, this section details the chronological construction of the system. For each sprint, we present the selected backlog objectives, the specific architectural decisions required for that increment, and the concrete deliverables produced at the end of the iteration.

% ================= SPRINT 0 =================
\subsection{Sprint 0 --- Research and Technical Bootcamp (Weeks 1--4)}

\subsubsection{Sprint Backlog}
Sprint 0 was a preparation sprint with no deployable code increment. Its objective was to build the technical and domain knowledge required to begin active development with confidence. The primary tasks included a technical bootcamp on LLM integration into offensive security workflows, a comparative analysis of existing AI-assisted penetration testing solutions, and hands-on practice with core tools (Nmap, Burp Suite, ffuf, SQLMap).

\subsubsection{Design \& Decisions}
During this phase, Scrum was confirmed as the development methodology. The core technology pair of FastAPI and Tauri was selected. Additionally, the three candidate LLMs (Gemini, Mistral, Qwen) were identified for benchmarking.

\subsubsection{Implementation \& Deliverables}
This sprint delivered the foundational technical knowledge base, the comparative analysis document (detailed in Chapter 2), the initial architecture sketch, and the formal product backlog.

% ================= SPRINT 1 =================
\subsection{Sprint 1 --- Core Architecture and Persistence Infrastructure (Weeks 5--6)}

\subsubsection{Sprint Backlog}
The objective of this first active sprint was to establish the foundational infrastructure of the platform.
\begin{itemize}
    \item \textbf{US01 \& US02:} Implement project creation and persistent project state.
    \item \textbf{US03:} Build a unified LLM gateway to route inference calls.
    \item \textbf{US04:} Set up the vector knowledge base for RAG ingestion.
\end{itemize}

\subsubsection{Design \& Decisions}
To maintain a lightweight local deployment, SQLite was chosen for project persistence over flat files, while Qdrant was selected for vector retrieval. Redis was introduced specifically for hot runtime coordination. A critical architectural decision was made to abstract provider selection from the agent code by creating the \texttt{LLMClient} gateway.

\subsubsection{Implementation \& Deliverables}
The sprint delivered a running FastAPI backend with a health endpoint, the SQLite projects store, and Redis/Qdrant containers running via Docker Compose. The \texttt{LLMClient} was implemented to support Ollama (for local execution) and the Mistral AI API, alongside an Intel Agent capable of document embedding.

% ================= SPRINT 2 =================
\subsection{Sprint 2 --- Planning Engine and Orchestration (Weeks 7--8)}

\subsubsection{Sprint Backlog}
With persistence active, Sprint 2 introduced the autonomous planning capabilities.
\begin{itemize}
    \item \textbf{US05:} Generate step-by-step, PTES-aligned attack plans.
    \item \textbf{US06:} Build the asynchronous scan orchestrator.
    \item \textbf{US07:} Develop the Architect agent.
\end{itemize}

\subsubsection{Design \& Decisions}
A major engineering choice was made to separate the "Checklist" and the "Plan" into two distinct persistence artifacts to enable smart resume logic. The orchestrator was designed as a layered service flow rather than a monolithic script. The Architect was deliberately kept as an LLM-backed node rather than a full agent to avoid unnecessary and expensive reasoning loops.

\subsubsection{Implementation \& Deliverables}
This increment produced the Planner agent, the centralized Scan Orchestrator (coordinating persistence, events, and runners), and the Architect node responsible for producing structured payloads and code suggestions.

% ================= SPRINT 3 =================
\subsection{Sprint 3 --- Execution Layer and Tool Sandbox (Weeks 9--10)}

\subsubsection{Sprint Backlog}
Sprint 3 delivered the sandboxed execution infrastructure, allowing the AI to safely interact with the target environment.
\begin{itemize}
    \item \textbf{US08:} Build an isolated tool sandbox.
    \item \textbf{US09:} Implement automated tool output parsing.
    \item \textbf{US10:} Enforce execution scope.
\end{itemize}

\subsubsection{Design \& Decisions}
To preserve the separation between the control plane and execution plane, the Sandbox was exposed as a completely separate HTTP service rather than a direct library call. \texttt{run\_custom} guardrails were implemented to classify tools, detect role violations, and validate network scope. A project-scoped tool result cache was introduced to prevent repeated identical executions.

\subsubsection{Implementation \& Deliverables}
The sprint delivered the Docker-based tool sandbox service (with \texttt{run\_custom} and \texttt{run\_python} routes), the Recon and Exploit Executor agents, automated output parsers for core tools (Nmap, ffuf, SQLMap, Nikto), and the project-scoped tool result cache.

% ================= SPRINT 4 =================
\subsection{Sprint 4 --- Analysis and Reporting Agents (Weeks 11--12)}

\subsubsection{Sprint Backlog}
Sprint 4 closed the active scan loop by delivering the intelligence filter and reporting capabilities.
\begin{itemize}
    \item \textbf{US11:} Filter false positives via an Analyzer agent.
    \item \textbf{US12:} Generate professional Markdown and HTML reports.
    \item \textbf{US13:} Implement a prompt injection guard.
\end{itemize}

\subsubsection{Design \& Decisions}
The Analyzer was architected with dual LLM runners (Verification and PoC Generation) to separate classification concerns from evidence synthesis. The SSVC decision tree was integrated into the enrichment path instead of relying solely on context-free CVSS scores. The Prompt Injection Guard was implemented as middleware on the prompt construction path.

\subsubsection{Implementation \& Deliverables}
Delivered the Analyzer agent with CVSS/SSVC enrichment, a Report Generator producing professional deliverables, the Prompt Injection Guard, and a project-scoped finding store supporting multiple states (verified, false positive, inconclusive, informational).

% ================= SPRINT 5 =================
\subsection{Sprint 5 --- Interface Layer and Human-in-the-Loop (Weeks 13--14)}

\subsubsection{Sprint Backlog}
Sprint 5 delivered the operator-facing desktop application and the critical control layers.
\begin{itemize}
    \item \textbf{US14:} Build the desktop UI.
    \item \textbf{US15:} Implement Human-in-the-Loop (HITL) approval gates.
    \item \textbf{US16:} Develop the context-aware AI assistant.
\end{itemize}

\subsubsection{Design \& Decisions}
Tauri was chosen over Electron to minimize the runtime footprint. Server-Sent Events (SSE) were adopted over standard polling to preserve real-time scan observability. HITL approval gates were modeled as persisted approval states that halt the orchestrator until operator resolution.

\subsubsection{Implementation \& Deliverables}
This sprint delivered the React 19 and Tauri desktop application (Projects, Dashboard, Scan, and Reports). This included the SSE-based live event stream, HITL approval banners for sensitive actions, and an Assistant agent featuring RAG-backed project context.

% ================= SPRINT 6 =================
\subsection{Sprint 6 --- Integration, Hardening, and Packaging (Weeks 15--16)}

\subsubsection{Sprint Backlog}
The final sprint focused on validating the full end-to-end platform, hardening resilience, and preparing the deployment package.
\begin{itemize}
    \item \textbf{US17 \& US18:} Implement smart scan resume and password-protected exports.
    \item \textbf{US19:} Create a client share link portal.
    \item \textbf{US20:} Package the platform via Docker Compose.
\end{itemize}

\subsubsection{Design \& Decisions}
The smart resume policy was formalized into a strict state machine with explicit checkpoints (initialized, gathering, checklist\_ready, plan\_ready, executing, completed). LLM rate-limiting resilience was hardened using exponential backoff. To ensure security during delivery, client share tokens were scoped exclusively to report content, completely isolating internal scan states.

\subsubsection{Implementation \& Deliverables}
Delivered the final checkpoint-based smart resume logic, password-protected PDF/HTML export packages (via 7-Zip integration), the client-facing scoped share portal, and the final Docker Compose stack deploying all six platform services from a single command.
% ── Section 4 ─────────────────────────────────────────────────────────────
...
\subsection{Scan Orchestration with LangGraph}

The core orchestration of the implemented platform is built on
\textbf{LangGraph}~\cite{langgraph}, a graph-based agent orchestration framework
that models the scan lifecycle as a directed state machine. Rather than a single procedural script, the scan runtime is expressed as a graph of nodes and conditional edges. Each node encapsulates a phase of the engagement, such as Intel preparation, memory construction, planning, execution, analysis, and reporting, while each edge encodes the transition condition that determines whether the pipeline advances, retries, or halts.

LangGraph was selected over a plain service orchestrator for three reasons specific
to PentaForge's requirements. First, its state-machine model maps directly onto the
smart resume logic: because each node persists its output before transitioning,
an interrupted scan can be resumed from the last completed node without replaying
earlier phases. Second, its conditional routing allows the orchestrator to branch dynamically based on analyzer output, such as looping back to the planner when a verified finding opens a new attack path, or escalating to a human approval gate before a sensitive tool is dispatched. Third, its native support for asynchronous node execution allows the Recon Executor and Exploit Executor to
progress independently on separate scenario branches without requiring manual
threading coordination.

At the API level, scan requests are accepted through the \texttt{/api/scans/start}
route. The request is handed to the LangGraph scan graph, which coordinates a
persistence service, an event service, an approval service, a phase runner, and a
lifecycle service through its node sequence. This decomposition gives the platform a
well-defined execution contract: every scan starts as a persisted runtime state and
evolves through explicit, auditable phase transitions.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{langgraph_flow.png}
    \caption{LangGraph-based scan state machine showing the node sequence
    and conditional transitions across the PentaForge execution lifecycle}
    \label{fig:langgraph_flow}
\end{figure}

Operationally, the runner node constructs worker callbacks for the recon lane,
...
node-by-node progression of the LangGraph execution.
...
\begin{itemize}
    ...
    into the LangGraph planning node so that the next execution cycle is based on
    evidence rather than assumptions, closing the feedback loop within the state
    machine.
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.55\textwidth]{ui_assistant.png}
    \caption{AI Assistant interface with project-aware context}
    \label{fig:ui_assistant}
\end{figure}
...
platform: project creation, target-type selection, scan launch, report review, and
client delivery.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{ui_projects.png}
<<    \caption{Project management workspace}
    \label{fig:ui_projects}
\end{figure}

Because the UI is packaged with Tauri, the product preserves the familiarity of a
web interface while remaining distributed as a desktop tool suitable for local use
inside a controlled assessment environment.
...
The operator manages these deliverables through the report generation interface,
which provides controls for Markdown editing, HTML preview, and share-link
configuration, as illustrated in Figure~\ref{fig:report_gen}.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{ui_report_share.png}
    \caption{Report generation and sharing controls}
...
viewer for the client. The final view presented to the client is a restricted,
read-only portal, shown in Figure~\ref{fig:client_portal}.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{ui_report_share2.png}
    \caption{Client-facing scoped report portal}
    \label{fig:client_portal}
\end{figure}

On the export side, the report module packages protected downloads so that
...
entry form to the selected target type, removing the ambiguity found in generic
pentest tools that force every engagement into a single flat target field.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{ui_target_type.png}
    \caption{Target-type aware project creation}
    \label{fig:ui_setting}
\end{figure}

\subsection{Platform Settings and AI Provider Configuration}

To support the requirement for flexible reasoning backbones and strict data sovereignty, PentaForge exposes a dedicated configuration workspace. Through this interface, the operator can seamlessly switch between AI providers, such as a local privacy-preserving model (e.g., Qwen via Ollama) or a high-capacity cloud provider (e.g., Gemini). It also centralizes the management of API keys and global platform parameters, ensuring the system remains adaptable to different engagement
constraints without requiring backend service restarts.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{ui_settings.png}
    \caption{Platform configuration and AI provider management interface}
...
In practice, this means that the implementation does not treat command execution as a simple shell wrapper. It treats it as a policy-checked action with context, target, role, and artifact semantics. This is a critical property for a security product, because the absence of such checks would make the platform itself a risk.
...
\mbox{containerized} environment with the full operational path enabled: desktop UI,
...
backend explicitly emits, as shown in the active scan dashboard (Figure \ref{fig:ui_scan_dashboard}).

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{ui_scan_dashboard.png}
    \caption{Active scan dashboard showing the orchestration pipeline and real-time execution events}
    \label{fig:ui_scan_dashboard}
\end{figure}
...
Standard platform operations, such as project creation, deletion, report regeneration, and protected downloads, were validated successfully and behave exactly as expected. To avoid diluting the evaluation with trivial CRUD operations, the functional test matrix below focuses strictly on the complex orchestration, agent reasoning, state-recovery, and data-isolation scenarios that validate PentaForge's core architecture.

\begin{longtable}{|p{0.08\textwidth}|p{0.24\textwidth}|p{0.28\textwidth}|p{0.30\textwidth}|}
\caption{Functional test matrix}\label{tab:functional_test_matrix}\\
\hline
\textbf{ID} & \textbf{Scenario} & \textbf{Expected result} & \textbf{Acceptance criterion} \\
\hline
\endfirsthead
\caption[]{Functional test matrix (continued)}\\
\hline
\textbf{ID} & \textbf{Scenario} & \textbf{Expected result} & \textbf{Acceptance criterion} \\
\hline
\endhead
T01 & Start a scan on a valid target & Pipeline enters running state and emits live updates. & UI shows planner, executor, analyzer, timer, and latest messages. \\
\hline
T02 & Generate checklist & Checklist is created from target type and project context. & Checklist items are saved and remain available after refresh. \\
\hline
T03 & Generate plan after checklist & Plan is created from checklist items. & Scenario identifiers are stable and visible in stored state. \\
\hline
T04 & Execute scenario & Tool request is routed through the sandbox. & Raw output, exit code, and normalized result are stored. \\
\hline
T05 & Analyze evidence & Evidence is classified correctly. & Verified findings require deterministic evidence, while noise remains informational or inconclusive. \\
\hline
T06 & Stop and resume before checklist & Incomplete scan starts from scratch. & No unstable partial data is reused as completed work. \\
\hline
T07 & Stop after checklist before plan & Plan is created on resume. & Checklist is preserved and execution continues. \\
\hline
T08 & Stop during scenario execution & Running scenario restarts from executor. & Completed scenarios are not repeated unless manually requested. \\
\hline
T09 & Generate report with no findings & Report states no verified findings. & It includes methodology, coverage, limitations, and project history without invented vulnerabilities. \\
\hline
T10 & Same target in two projects & Reports remain project-scoped. & Findings from one project do not appear in another project unless generated there. \\
\hline
% caption and label are defined at the top of the longtable
\end{longtable}
...
a controlled \mbox{containerized} environment with limited access. \\
\hline
...
% ── Penetration Testing Methodology & Lifecycle ────────────────────────────

\bibitem{triaxiomTimeline}
Triaxiom Security.
\textit{What is the Typical Timeline for a Penetration Test?}
Available at: \url{https://www.triaxiomsecurity.com/blog/typical-timeline-for-a-penetration-test/}
(accessed May 2026).

\bibitem{valuementorLifecycle}
ValueMentor.
\textit{Penetration Testing Service Lifecycle: Inside the Complete Assessment}.
Available at: \url{https://valuementor.com/blogs/penetration-testing-service-inside-the-complete-assessment-lifecycle}
(accessed May 2026).

\bibitem{ptes}
Penetration Testing Execution Standard (PTES).
\textit{PTES Technical Guidelines}.
Available at: \url{http://www.pentest-standard.org/index.php/Main_Page}
(accessed May 2026).

\bibitem{mitreattack}
MITRE Corporation.
\textit{MITRE ATT\&CK: Adversarial Tactics, Techniques and Common Knowledge Framework}.
Available at: \url{https://attack.mitre.org}
(accessed May 2026).

\bibitem{cycognitoPentest}
CyCognito.
\textit{Penetration Testing in 2025: Challenges, Trends and Automation}.
Available at: \url{https://www.cycognito.com/learn/penetration-testing/}
(accessed May 2026).

% ── AI in Penetration Testing & Automation ────────────────────────────────

\bibitem{pentestgpt2023}
G.~Deng, Y.~Liu, V.~Mayoral-Vilches, P.~Liu, Y.~Li, Y.~Xu, T.~Zhang,
Y.~Liu, M.~Pinzger, and S.~Rass.
\textit{PentestGPT: Evaluating and Harnessing Large Language Models for
Automated Penetration Testing}.
arXiv preprint arXiv:2308.06782, 2023.
Available at: \url{https://arxiv.org/abs/2308.06782}
(accessed May 2026).

\bibitem{autosecagent2026}
H.~Al-Shaaby, A.~Alqahtani, and M.~Zohdy.
\textit{AutoSecAgent: An Autonomous Multi-Agent Framework for Automated
Cybersecurity Operations}.
\textit{The Journal of Supercomputing}, Springer Nature, 2026.
Available at: \url{https://link.springer.com/article/10.1007/s11227-026-08439-z}.

\bibitem{bishopfoxAI}
Bishop Fox.
\textit{AI Tooling Impact on Penetration Testing Engagements},
cited in: RedFox Security, \textit{The State of AI Pentesting in 2026:
Trends, Statistics and What's Next}.
Available at: \url{https://www.redfoxsec.com/blog/the-state-of-ai-pentesting-in-2026-trends-statistics-and-whats-next}
(accessed May 2026).

\bibitem{brightdefense2026}
Bright Defense.
\textit{120+ Penetration Testing Statistics for 2026}.
Available at: \url{https://www.brightdefense.com/resources/penetration-testing-statistics}
(accessed May 2026).

\bibitem{emazzantiTools}
eMazzanti Technologies.
\textit{Benefits and Limitations of Automated Tools in Penetration Testing}.
Available at: \url{https://www.emazzanti.net/benefits-and-limitations-of-automated-tools-in-penetration-testing/}
(accessed May 2026).

% ── Existing Automated Pentest Solutions ──────────────────────────────────

\bibitem{burpsuiteAI}
PortSwigger.
\textit{Burp Suite AI: Explore Issue and AI-Assisted Testing Features}.
Available at: \url{https://portswigger.net/burp/documentation/desktop/ai-features}
(accessed May 2026).

\bibitem{kaligpt}
M.~Valk.
\textit{KaliGPT: AI-Assisted Penetration Testing on Kali Linux}.
GitHub repository, 2024.
Available at: \url{https://github.com/markusvalk/kaligpt}
(accessed May 2026).

\bibitem{nodezero}
Horizon3.ai.
\textit{NodeZero: Autonomous Penetration Testing Platform}.
Available at: \url{https://www.horizon3.ai/nodezero}
(accessed May 2026).

\bibitem{shannon}
Keygraph.
\textit{Shannon: Open-Source White-Box AI Pentester for Web Applications}.
GitHub repository, 2024.
Available at: \url{https://github.com/keygraph/shannon}
(accessed May 2026).
...
ThreatExploit AI.
\textit{The Hidden Cost of False Positives: Why Scanners Alone Are Not Enough}.
Available at: \url{https://threatexploit.ai/en/blog/false-positives-scanners-vs-pentesting}
(accessed May 2026).

\bibitem{hedgehogRetesting}
Hedgehog Security.
\textit{Remediation Validation and Retesting: Closing the Loop}.
Available at: \url{https://www.hedgehogsecurity.co.uk/blog/remediation-validation-retesting-close-the-loop}
(accessed May 2026).

\bibitem{redleggRetesting}
RedLegg.
\textit{The Role of Retesting in Vulnerability Remediation Validation}.
Available at: \url{https://www.redlegg.com/blog/vulnerability-remediation-retesting-guide}
(accessed May 2026).

\bibitem{halockVerification}
HALOCK Security Labs.
\textit{Remediation Verification: Confirming Vulnerabilities Are Truly Fixed}.
Available at: \url{https://www.halock.com/penetration-testing/remediation-verification/}
(accessed May 2026).
...
Available at: \url{https://arxiv.org/abs/2412.15115}.

\bibitem{geminiRateLimits}
Google DeepMind.
\textit{Gemini API: Rate Limits, Usage Tiers and Quotas}.
Available at: \url{https://ai.google.dev/gemini-api/docs/rate-limits}
(accessed May 2026).

\bibitem{llmstatsFlash}
LLM Stats.
\textit{Gemini 2.5 Flash vs.\ Gemini 2.5 Pro: Benchmark Comparison}.
Available at: \url{https://llm-stats.com/models/compare/gemini-2.5-flash-vs-gemini-2.5-pro}
(accessed May 2026).
Note: Global MMLU-Lite score for Gemini~2.5~Flash reported as 88.4\%.

\bibitem{mistralDocs}
Mistral AI.
\textit{API Rate Limits and Usage Tiers}.
Available at: \url{https://docs.mistral.ai/admin/user-management-finops/tier}
(accessed May 2026).
...
\bibitem{langgraph}
LangChain, Inc.
\textit{LangGraph: A Library for Building Stateful, Multi-Actor LLM Applications}.
Available at: \url{https://langchain-ai.github.io/langgraph/}
(accessed May 2026).

\bibitem{pmpartnersScrumProcess}
PM Partners.
\textit{Scrum Process} (image).
Available at: \url{https://share.google/5HtFyjvGZg3le6mXj}
(accessed June 2026).

\bibitem{talanLogo}
Talan.
\textit{Talan Logo} (image).
Available at: \url{https://share.google/cRQadkwXqYaxIiegY}
(accessed June 2026).
