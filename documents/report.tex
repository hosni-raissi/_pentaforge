...
PentaForge addresses this by maintaining per-domain vector indexes ---
...
Providing a single number without deployment context misleads triage and
...
The requirements defined in this section --- functional, non-functional, and
actor-oriented --- collectively specify the contract that PentaForge must
fulfill. The following section translates these requirements into a
prioritized delivery plan through the product backlog.
...
\begin{figure}[H]
    \centering
    \makebox[\textwidth][c]{%
        \includegraphics[width=1.1\textwidth]{concept_architecture.png}%
    }
...
exercise a distinct and critical path through the platform --- scan initiation,
failure recovery, report production, and secure client delivery --- collectively
covering the full engagement lifecycle from the operator's first action to the
client's final download.
...
\begin{figure}[H]
    ...
    \caption{Sequence Diagram: Project Scan Launch}
...
\begin{figure}[H]
    ...
    \caption{Sequence Diagram: Resume After Interruption}
...
\begin{figure}[H]
    ...
    \caption{Sequence Diagram: Protected Client Download}
...
design decision was made. The three architectural views --- monolithic, logical,
and physical --- established the structural boundaries from a single deployable
unit down to individual containers and their network relationships. The deep
...
\subsubsection{Implementation \& Deliverables}
The sprint delivered a running FastAPI backend with a health endpoint, the SQLite projects store, and Redis/Qdrant containers running via Docker Compose. The \texttt{LLMClient} was implemented to support Ollama and Mistral, alongside an Intel Agent capable of document embedding.
...
\subsubsection{Implementation \& Deliverables}
The sprint delivered the Docker-based tool sandbox service (with \texttt{run\_custom} and \texttt{run\_python} routes), the Recon and Exploit Executer agents, automated output parsers for core tools (Nmap, ffuf, SQLMap, Nikto), and the project-scoped tool result cache.
...
\subsubsection{Implementation \& Deliverables}
The release of the React 19 and Tauri desktop application (Projects, Dashboard, Scan, Reports). This included the SSE-based live event stream, HITL approval banners for sensitive actions, and an Assistant agent featuring RAG-backed project context.
...
that models the scan lifecycle as a directed state machine. Rather than a single
procedural script, the scan runtime is expressed as a graph of nodes and conditional
edges, where each node encapsulates a phase of the engagement --- Intel preparation,
memory construction, planning, execution, analysis, and reporting --- and each edge
encodes the transition condition that determines whether the pipeline advances,
retries, or halts.
...
earlier phases. Second, its conditional routing allows the orchestrator to branch
dynamically based on analyzer output --- for example, looping back to the planner
when a verified finding opens a new attack path, or escalating to a human approval
gate before a sensitive tool is dispatched. Third, its native support for
asynchronous node execution allows the Recon Executer and Exploit Executer to
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{ui_report_share.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{ui_report_share2.png}
...
To support the requirement for flexible reasoning backbones and strict data
sovereignty, PentaForge exposes a dedicated configuration workspace. Through this
interface, the operator can seamlessly switch between AI providers --- such as a
local privacy-preserving model (e.g., Qwen via Ollama) or a high-capacity cloud
provider (e.g., Gemini). It also centralizes the management of API keys and global
platform parameters, ensuring the system remains adaptable to different engagement
constraints without requiring backend service restarts.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{ui_settings.png}
...
\hline
\endhead
T01 & Start a scan on a valid target & Pipeline enters running state and emits live updates. & UI shows planner, executer, analyzer, timer, and latest messages. \\
\hline
...
\hline
T08 & Stop during scenario execution & Running scenario restarts from executer. & Completed scenarios are not repeated unless manually requested. \\
\hline
