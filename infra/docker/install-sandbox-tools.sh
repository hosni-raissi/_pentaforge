...
run_best_effort "apt-amass" bash -lc 'apt-get update && apt-get install -y --no-install-recommends amass'
run_best_effort "apt-masscan" bash -lc 'apt-get update && apt-get install -y --no-install-recommends masscan'
run_best_effort "apt-nikto" bash -lc 'apt-get update && apt-get install -y --no-install-recommends nikto'
run_best_effort "apt-whatweb" bash -lc 'apt-get update && apt-get install -y --no-install-recommends whatweb'
run_best_effort "apt-network-stack" bash -lc 'apt-get update && apt-get install -y --no-install-recommends arp-scan dnsrecon enum4linux fping ike-scan ldap-utils mtr nbtscan netdiscover nfs-common onesixtyone proxychains4 rpcbind smbclient snmp snmpcheck tcpdump tshark'
run_best_effort "besttrace-install" bash -lc '
...
'
run_best_effort "rustscan-install" bash -lc '
  VERSION=$(curl -s https://api.github.com/repos/RustScan/RustScan/releases/latest | grep tag_name | cut -d "\"" -f 4)
...
'
run_best_effort "apt-mobile-stack" bash -lc 'apt-get update && apt-get install -y --no-install-recommends apktool binwalk default-jdk-headless'
run_best_effort "apt-container-cloud-stack" bash -lc 'apt-get update && apt-get install -y --no-install-recommends awscli kubernetes-client skopeo yq'
run_best_effort "apt-sqlmap" bash -lc 'apt-get update && apt-get install -y --no-install-recommends sqlmap'
run_best_effort "apt-exploitation" bash -lc 'apt-get update && apt-get install -y --no-install-recommends hydra john openssh-client ftp'
...
run_best_effort "pip-recon-core" python3 -m pip install --prefer-binary \
  wafw00f \
  mitmproxy \
  theHarvester \
  recon-ng \
  ...
  censys \
  sshuttle
run_best_effort "ligolo-ng-agent-install" bash -lc '
  VERSION=$(curl -s https://api.github.com/repos/nicocha30/ligolo-ng/releases/latest | grep tag_name | cut -d "\"" -f 4)
...
run_best_effort "apt-azure-cli" bash -lc '
...
'
run_best_effort "apt-gcloud-sdk" bash -lc '
...
'
run_best_effort "apt-docker-cli" bash -lc '
...
'
run_best_effort "apt-nomad" bash -lc '
  curl -fsSL https://apt.releases.hashicorp.com/gpg | gpg --dearmor -o /usr/share/keyrings/hashicorp-archive-keyring.gpg
  echo "deb [signed-by=/usr/share/keyrings/hashicorp-archive-keyring.gpg] https://apt.releases.hashicorp.com $(lsb_release -cs) main" > /etc/apt/sources.list.d/hashicorp.list
  apt-get update && apt-get install -y --no-install-recommends nomad
'
run_best_effort "kube-bench-install" bash -lc '
  VERSION=$(curl -s https://api.github.com/repos/aquasecurity/kube-bench/releases/latest | grep tag_name | cut -d "\"" -f 4)
...
'
run_best_effort "calicoctl-install" bash -lc '
...
'
run_best_effort "clair-scanner-install" bash -lc '
  VERSION=$(curl -s https://api.github.com/repos/arminc/clair-scanner/releases/latest | grep tag_name | cut -d "\"" -f 4)
...
'
run_best_effort "reg-install" bash -lc '
  VERSION=$(curl -s https://api.github.com/repos/genuinetools/reg/releases/latest | grep tag_name | cut -d "\"" -f 4)
...
'
run_best_effort "kubenscan-install" bash -lc '
...
'
run_best_effort "apt-gh-cli" bash -lc '
  curl -fsSL https://cli.github.com/packages/githubcli-archive-keyring.gpg | gpg --dearmor -o /usr/share/keyrings/githubcli-archive-keyring.gpg
  echo "deb [arch=$(dpkg --print-architecture) signed-by=/usr/share/keyrings/githubcli-archive-keyring.gpg] https://cli.github.com/packages stable main" > /etc/apt/sources.list.d/github-cli.list
  apt-get update && apt-get install -y --no-install-recommends gh
'
run_best_effort "apt-glab-cli" bash -lc '
  curl -sSL https://packages.gitlab.com/install/repositories/gitlab/gitlab-explorer/script.deb.sh | bash
  apt-get install -y glab
'
run_best_effort "codeql-install" bash -lc '
  VERSION=$(curl -s https://api.github.com/repos/github/codeql-cli-binaries/releases/latest | grep tag_name | cut -d "\"" -f 4)
...
'
run_best_effort "pip-s3scanner" python3 -m pip install --prefer-binary s3scanner
run_best_effort "pip-web-api-tools" python3 -m pip install --prefer-binary \
  arjun \
  zap-cli \
  param-miner \
  paramspider \
  ...
  droopescan \
  pyntcli
run_best_effort "pip-network-repo-tools" python3 -m pip install --prefer-binary \
  detect-secrets \
  knockpy \
  ldapdomaindump \
  bloodhound \
  repo-supervisor \
  ssh-audit
run_best_effort "pip-ad-tools" python3 -m pip install --prefer-binary \
  impacket \
  smbmap \
  crackmapexec \
  netexec \
  enum4linux-ng
run_best_effort "pip-mobile-runtime-tools" python3 -m pip install --prefer-binary \
  frida-tools \
  objection
run_best_effort "pip-cloud-container-tools" python3 -m pip install --prefer-binary \
  kube-hunter \
  scoutsuite
run_best_effort "pip-cloud-aws-tools" python3 -m pip install --prefer-binary pacu
...
run_best_effort "whaler" install_go_tool whaler github.com/Pega-sys/whaler@latest
...
run_best_effort "kiterunner" install_go_tool kr github.com/assetnote/kiterunner@latest
run_best_effort "grpcurl" install_go_tool grpcurl github.com/fullstorydev/grpcurl/cmd/grpcurl@latest
run_best_effort "zgrab2" install_go_tool zgrab2 github.com/zmap/zgrab2@latest
run_best_effort "chisel" install_go_tool chisel github.com/jpillora/chisel@latest
run_best_effort "ctop" install_go_tool ctop github.com/bcicen/ctop@latest
...
run_best_effort "grype" install_go_tool grype github.com/anchore/grype@latest
run_best_effort "dive" install_go_tool dive github.com/wagoodman/dive@latest
run_best_effort "gitleaks" install_go_tool gitleaks github.com/gitleaks/gitleaks/v8@latest
run_best_effort "trufflehog" install_go_tool trufflehog github.com/trufflesecurity/trufflehog/v3@latest
...
run_best_effort "diggit" install_go_tool diggit github.com/derision-test/diggit@latest
run_best_effort "cloudbrute" install_go_tool cloudbrute github.com/0xsha/CloudBrute@latest

log "installing system security tools..."
run_best_effort "apt-security-core" bash -lc 'apt-get update && apt-get install -y --no-install-recommends nmap amap hashcat metasploit-framework netcat-traditional protobuf-compiler'

log "installing binary utilities..."
run_best_effort "pspy-install" bash -lc 'curl -sfL https://github.com/DominicBreuker/pspy/releases/latest/download/pspy64 -o /usr/local/bin/pspy && chmod +x /usr/local/bin/pspy'
...
run_best_effort "massdns-install" bash -lc '
...
run_best_effort "cmseek-deps" python3 -m pip install -r "${TOOLS_ROOT}/cmseek/requirements.txt"
write_python_wrapper "cmseek" "${TOOLS_ROOT}/cmseek/cmseek.py"

run_best_effort "feroxbuster-install" bash -lc '
...
run_best_effort "stormspotter-install" python3 -m pip install --prefer-binary stormspotter
...
run_best_effort "oauth-scanner-repo" clone_repo https://github.com/p0dalirius/OAuth-Scanner.git "${TOOLS_ROOT}/OAuth-Scanner"
run_best_effort "kubescan-install" bash -lc '
...
run_best_effort "blobenum-repo" clone_repo https://github.com/skypLabs/blobenum.git "${TOOLS_ROOT}/blobenum"
...
run_best_effort "pulumi-install" bash -lc 'curl -fsSL https://get.pulumi.com | sh'
run_best_effort "steampipe-install" bash -lc 'sh -c "$(curl -fsSL https://steampipe.io/install/steampipe.sh)"'
...
run_best_effort "lse-install" bash -lc 'curl -sfL https://raw.githubusercontent.com/diego-treitos/linux-smart-enumeration/master/lse.sh -o /usr/local/bin/lse && chmod +x /usr/local/bin/lse'
run_best_effort "trivy-install" bash -lc 'curl -sfL https://raw.githubusercontent.com/aquasecurity/trivy/main/contrib/install.sh | sh -s -- -b /usr/local/bin'
run_best_effort "wpscan-install" gem install wpscan

log "installing repo-specific python deps..."
run_best_effort "linkfinder-deps" bash -lc 'test ! -f "'"${TOOLS_ROOT}/LinkFinder/requirements.txt"'" || python3 -m pip install --prefer-binary -r "'"${TOOLS_ROOT}/LinkFinder/requirements.txt"'"'
run_best_effort "secretfinder-deps" bash -lc 'test ! -f "'"${TOOLS_ROOT}/SecretFinder/requirements.txt"'" || python3 -m pip install --prefer-binary -r "'"${TOOLS_ROOT}/SecretFinder/requirements.txt"'"'
run_best_effort "getjs-deps" bash -lc 'test ! -f "'"${TOOLS_ROOT}/getJS/requirements.txt"'" || python3 -m pip install --prefer-binary -r "'"${TOOLS_ROOT}/getJS/requirements.txt"'"'
run_best_effort "cloud-enum-deps" bash -lc 'test ! -f "'"${TOOLS_ROOT}/cloud_enum/requirements.txt"'" || python3 -m pip install --prefer-binary -r "'"${TOOLS_ROOT}/cloud_enum/requirements.txt"'"'
run_best_effort "corscanner-deps" bash -lc 'test ! -f "'"${TOOLS_ROOT}/CORScanner/requirements.txt"'" || python3 -m pip install --prefer-binary -r "'"${TOOLS_ROOT}/CORScanner/requirements.txt"'"'
run_best_effort "sqlmap-deps" bash -lc 'test ! -f "'"${TOOLS_ROOT}/sqlmap/requirements.txt"'" || python3 -m pip install --prefer-binary -r "'"${TOOLS_ROOT}/sqlmap/requirements.txt"'"'
run_best_effort "nosqlmap-deps" bash -lc 'test ! -f "'"${TOOLS_ROOT}/NoSQLMap/requirements.txt"'" || python3 -m pip install --prefer-binary -r "'"${TOOLS_ROOT}/NoSQLMap/requirements.txt"'"'
run_best_effort "jwt-tool-deps" bash -lc 'test ! -f "'"${TOOLS_ROOT}/jwt_tool/requirements.txt"'" || python3 -m pip install --prefer-binary -r "'"${TOOLS_ROOT}/jwt_tool/requirements.txt"'"'
run_best_effort "xsstrike-deps" bash -lc 'test ! -f "'"${TOOLS_ROOT}/XSStrike/requirements.txt"'" || python3 -m pip install --prefer-binary -r "'"${TOOLS_ROOT}/XSStrike/requirements.txt"'"'
run_best_effort "commix-deps" bash -lc 'test ! -f "'"${TOOLS_ROOT}/commix/requirements.txt"'" || python3 -m pip install --prefer-binary -r "'"${TOOLS_ROOT}/commix/requirements.txt"'"'
run_best_effort "tplmap-deps" bash -lc 'test ! -f "'"${TOOLS_ROOT}/tplmap/requirements.txt"'" || python3 -m pip install --prefer-binary -r "'"${TOOLS_ROOT}/tplmap/requirements.txt"'"'
run_best_effort "ssrfmap-deps" bash -lc 'test ! -f "'"${TOOLS_ROOT}/SSRFmap/requirements.txt"'" || python3 -m pip install --prefer-binary -r "'"${TOOLS_ROOT}/SSRFmap/requirements.txt"'"'
run_best_effort "blobenum-deps" bash -lc 'test ! -f "'"${TOOLS_ROOT}/blobenum/requirements.txt"'" || python3 -m pip install --prefer-binary -r "'"${TOOLS_ROOT}/blobenum/requirements.txt"'"'
run_best_effort "gcpbucketbrute-deps" bash -lc 'test ! -f "'"${TOOLS_ROOT}/GCPBucketBrute/requirements.txt"'" || python3 -m pip install --prefer-binary -r "'"${TOOLS_ROOT}/GCPBucketBrute/requirements.txt"'"'
run_best_effort "oauth-scanner-deps" bash -lc 'test ! -f "'"${TOOLS_ROOT}/OAuth-Scanner/requirements.txt"'" || python3 -m pip install --prefer-binary -r "'"${TOOLS_ROOT}/OAuth-Scanner/requirements.txt"'"'
run_best_effort "graphql-cop-deps" bash -lc 'test ! -f "'"${TOOLS_ROOT}/graphql-cop/requirements.txt"'" || python3 -m pip install --prefer-binary -r "'"${TOOLS_ROOT}/graphql-cop/requirements.txt"'"'
